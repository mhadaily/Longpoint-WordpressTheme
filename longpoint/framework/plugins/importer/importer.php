...
// RchTheme Demo Data Importer

// Hook importer into admin init
add_action( 'admin_init', 'rch_importer' );
function rch_importer() {
    global $wpdb;
    ...
    if ( current_user_can( 'manage_options' ) && isset( $_GET['import_data_content'] ) ) {
        ...
        if ( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ) { // check for main import class and wp import class
			...
			/* Import Posts, Pages, Portfolio Content, Elastic Slider, Parallax Sections, Images, Menus */
			$theme_xml = get_template_directory() . '/framework/plugins/importer/data/longpoint.wordpress.2014-06-21.xml';
          ...
          if (is_array(@default_sidebars)) {
            ...
            foreach ( @default_sidebars as $name => $title ) {
            ...
            $import_widgets = rch_import_widget_data( $widget_data );            
...
function rch_import_widget_data( $widget_data ) {
    ...
    rch_parse_import_data( $sidebar_data );
}

function rch_parse_import_data( $import_array ) {
    ...
    foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
        ...
        foreach ( $import_widgets as $import_widget ) :
            ...
            if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
                ...
                $new_widget_name = rch_get_new_widget_name( $title, $index );
...
function rch_get_new_widget_name( $widget_name, $widget_index ) {
    $current_sidebars = get_option( 'sidebars_widgets' );
