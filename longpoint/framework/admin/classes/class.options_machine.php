...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'sidebar':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_slidebar_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slidebar_add_button">Add New Slidebar</a></div>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ','Longpoint').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_media_uploader_function($id,$std,$mod){
		...
		$uploader .= '<input class="'.$hide.' upload of-input rch-input" name="'. $id .'" id="rch_'. $id .'_upload" value="'. $val .'" />';	
...
/**
	 ...
	 * @return string
	 */
	public static function optionsframework_slidebar_function($id,$std,$oldorder,$order){
		...
		//initialize all vars
		$slidevars = array('title','name','description');
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Slidebar '.$order.'</strong>';
		}
		...
		$slider .= '<input class="slide of-input of-slider-title" name="'. $id .'['.$order.'][name]" id="'. $id .'_'.$order .'_slide_name" value="'. stripslashes($val['name']) .'" />';
